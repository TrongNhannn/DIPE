...
router.put(`/changeinfo/:credential_string`, (req, res) => {
    ...
    mysql(query, result => {
        if (result) {
            res.status(200).send({ success: true, content: "Cập nhật thông tin thành công" })
        } else {
            res.status(500).send({ success: false, content: "Cập nhật thông tin thất bại" })
        }
    })
})
router.put(`/change/avatar/:credential_string`, (req, res) => {
    ...
    cropIMG(img, credential_string, ({ success, avatar }) => {
        ...
        mysql(query, result => {
            if (result) {
                res.status(200).send({ success: true, content: "Thay đổi ảnh đại diện thành công" })
            } else {
                res.status(500).send({ success: false, content: "Thay đổi ảnh thất bại" })
            }
        })
    })
})
